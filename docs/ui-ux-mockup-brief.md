...
**Update, 2026-08-17**: questions 3, 4, 5, 8, 9, 10, and 11 below are now resolved by direct product-owner answer (not inferred).
...
3. ~~**CSV import staging**~~ — **Resolved.** A true multi-step wizard: distinct Configure → Upload → Review screens with back/forward, not a single combined screen.
4. ~~**OCR capture UX**~~ — **Resolved.** Support both: capture via camera and picking an existing photo/PDF from the gallery/files.
5. ~~**Is the consent-acceptance screen in scope for this mockup round?**~~ — **Resolved: in scope.** No mockup exists for it — build it in the app's existing plain-screen style (matches the unmocked Portfolio Detail precedent), per the compliance framing in `docs/compliance-foundation.md`.
...
8. ~~**Password reset/change**~~ — **Resolved: scoped out of v1.** No such capability exists anywhere in the backend today; building it (email delivery, reset tokens, etc.) is new scope, not a missing Flutter screen. Revisit only if Piggybank moves beyond a single-user personal app.
9. ~~**Transaction category field**~~ — **Resolved: stays free text.** No picker. OCR/import auto-categorization may still *suggest* a category value without constraining the field.
10. ~~**Avatar photo + notification bell**~~ — **Resolved: stay as non-functional placeholders.** Consistent with the mascot/avatar image-asset decision (real assets not sourced this pass) — no avatar-upload endpoint or `notifications` domain gets built this pass either. `DESIGN.md`'s placeholder note applies to both the image asset and the backing feature.
11. ~~**6th nav tab or restructure**~~ — **Resolved: open, not fixed.** The 5-tab structure is not locked in — a 6th tab or nav restructure may happen once Investments' sub-areas exist and the real information architecture is clearer. No specific new structure decided yet.
