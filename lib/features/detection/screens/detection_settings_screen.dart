...
/// screen", Phase D — notifications only; the Gmail "Connect" half is
/// Phase E). Three things a user does here, top to bottom: accept the
/// feature's opt-in consent, grant "Notification access" in system
/// settings, then manage the allowlist itself.
...
class _DetectionSettingsScreenState extends ConsumerState<DetectionSettingsScreen> with WidgetsBindingObserver {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (consentAccepted) {
        sources = await ref.read(detectionApiProvider).listNotificationSources();
