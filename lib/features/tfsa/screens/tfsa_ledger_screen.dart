...
import '../providers/tfsa_provider.dart';

/// TFSA contribution ledger — per `ui-ux-mockup-brief.md` §5.5: pure
/// compliance/limit tracking (lifetime R500,000, annual R36,000), not
/// investment performance. Reached by pushing from a TFSA-type portfolio's
/// detail screen (the ledger itself is user-scoped, not portfolio-scoped,
/// per the backend's `/tfsa/*` endpoints — there is only ever one TFSA
/// portfolio per user, so any one of them is a valid entry point).
class TfsaLedgerScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('TFSA contribution ledger')),
      body: SafeArea(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => showAddContributionSheet(context),
...
class _SummarySection extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final summaryAsync = ref.watch(tfsaSummaryProvider);
        ...
        return Column(
          ...
          children: [
            HeroMetricCard(
              ...
              value: formatZAR(summary.lifetimeContributed),
            ...
            if (summary.yearsToLifetimeLimit != null) ...[
              const SizedBox(height: 8),
              Text(
                '${summary.yearsToLifetimeLimit} year${summary.yearsToLifetimeLimit == 1 ? '' : 's'} to lifetime limit '
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return summaryAsync.when(
      ...
      data: (summary) => GrowthProjectionCard(
        startingBalance: summary.lifetimeContributed.toDouble(),
        label: 'TFSA',
      ),
    );
  }
...
class _ContributionRow extends ConsumerWidget {
  ...
  final TfsaContribution contribution;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final subtitleParts = <String>[
      ...
      if (contribution.ticker != null) contribution.ticker!,
...
Future<void> showAddContributionSheet(BuildContext context) {
...
class _AddContributionSheetState extends ConsumerState<_AddContributionSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final ticker = _tickerController.text.trim().isEmpty ? null : _tickerController.text.trim();
      await ref.read(tfsaApiProvider).createContribution(
            taxYear: taxYear,
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Ticker (optional)'),
            ),
            const SizedBox(height: 16),
            TextField(
